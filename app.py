...
with tab1:
    ...
    if analyze_text:
        if not user_text or len(user_text.strip()) == 0:
            ...
            if conf_pct < 60:
                label = "uncertain"
                st.markdown(f'<div class="result-uncertain">UNCERTAIN<div class="result-sub">Low Confidence — Result may not be reliable. Try providing more content.</div></div>', unsafe_allow_html=True)
            elif ai_pct > 60:
                label = "ai"
...
# ═══════════════════════════════════════════════════════════════════════════════
with tab2:
    st.markdown('<p class="section-label">Uses EfficientNet B0 — fine-tuned on COCOAI dataset</p>', unsafe_allow_html=True)
    ...
    if uploaded_file:
        ...
        with col2:
            ...
            if analyze_img:
                ...
                if conf_pct < 60:
                    label = "uncertain"
                    st.markdown('<div class="result-uncertain">UNCERTAIN<div class="result-sub">Low confidence — try a clearer image</div></div>', unsafe_allow_html=True)
                elif ai_pct > 60:
                    label = "ai"
...
st.markdown("""
<div class="footer">
    AI-Generated Content Detection System &nbsp;|&nbsp; 2026<br>
    · RoBERTa · EfficientNet B0
</div>
""", unsafe_allow_html=True)
